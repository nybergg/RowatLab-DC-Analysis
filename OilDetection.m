...
function processed = OilDetection(currVideo, startFrame, endFrame, currVideoName, mask)
...
% folderName = 'Y:\Kendra\HL60 Microfluidics\140803 - WT HL0 Drug Treatments I\Blb\';
% currVideoName = 'dev5x5_200fps_2dppt_1o10PDMS_0,1F127_20x_0.4ms__35umfilter_022.avi';
% 
...
% 
% filters = [500; 3; 35; 10; 5];
% 
% lowContrast = userPref(5);
...
% creates filters for oil detection
filters = [500; 3; 35; 20; 5];
...
for frameIdx = startFrame:endFrame
    ...
    % Cell detection
    if (frameIdx == 1)
        % converts first frame to bw with threshold and without background subtraction
        cleanImg = im2bw(currFrame(1:60,:), graythresh(currFrame)*1.1);
    else
        % subtracts the background in bgImgs from each frame
        % oil droplets
         cleanImg = imsubtract(backgroundImg*.85, currFrame)*20;
    end
